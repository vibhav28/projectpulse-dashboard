  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="border-b px-6 py-3 shrink-0">
        ...
        <p className="text-xs text-muted-foreground">Ask anything about your project data</p>
      </div>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-sm">
              Ask questions about your uploaded datasets or connected Jira projects.
            </p>
          </div>
        )}
              ...
              >
                ...
                {msg.sources && msg.sources.length > 0 && (
                  <div className="mt-3 pt-2 border-t border-border/50">
                    ...
                    >
                      {expandedSources.has(i) ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                      {msg.sources.length} source{msg.sources.length > 1 ? "s" : ""}
                    </button>
                    {expandedSources.has(i) && (
                      <div className="mt-2 space-y-2">
                        {msg.sources.map((src, j) => (
                          <div key={j} className="rounded-lg bg-background/50 p-2.5 text-xs">
                            {src.title && <p className="font-medium mb-1">{src.title}</p>}
                            <p className="opacity-80 line-clamp-3">{src.content || JSON.stringify(src)}</p>
                          </div>
                        ))}
                      </div>
                    )}
